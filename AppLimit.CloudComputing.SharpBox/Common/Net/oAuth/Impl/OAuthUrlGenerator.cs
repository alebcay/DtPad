...
namespace AppLimit.CloudComputing.SharpBox.Common.Net.oAuth.Impl
{
    internal class OAuthUrlGenerator
    {
        ...
        static public String GenerateAuthorizationUrl(String baseAuthorizeUrl, String callbackUrl, OAuthToken requestToken)
        {
            ...
            sb.AppendFormat("?oauth_token={0}", Uri.EscapeDataString(requestToken.TokenKey)); //HttpUtility.UrlEncode(requestToken.TokenKey));
            sb.AppendFormat("&oauth_callback={0}", Uri.EscapeDataString(callbackUrl)); //HttpUtility.UrlEncode(callbackUrl));
        ...
        static private String GenerateSignedUrl(String baseRequestTokenUrl, ValidRequestMethod method, OAuthConsumerContext context, OAuthToken token)
        {
            ...
            //
            sig = Uri.EscapeDataString(sig); //HttpUtility.UrlEncode(sig);
