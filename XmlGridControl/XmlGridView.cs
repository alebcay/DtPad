...
namespace WmHelp.XmlGrid
{
    public class XmlGridView: Control
    {
        ...
        {
            return col < _columnsWidth.Length ? _columnsWidth[col] : 0;
        }

        protected int RowHeight(int row)
        {
            return row < _rowHeight.Length ? _rowHeight[row] : 0;
        }
        ...
        protected void SetTableRows(GridCellGroup cell)
        {
            if (cell.Expanded)
            {
                for (int s = 0; s < cell.Table.Height; s++)
                {
                    ...
                    for (int k = 0; k < cell.Table.Width; k++)
                    {
                        cell.Table.RowCount[s] = Math.Max(cell.Table.RowCount[s], CountCellRows(cell.Table[k, s]));
                        if (cell.Table[k, s].IsGroup)
                        {
        ...
        protected void DoAutoHeightCells(int row, int count, GridCellGroup cell, Graphics g)
        {
            ...
            for (int s = 0; s < cell.Table.Height; s++)
            {
                ...
                for (int k = 0; k < cell.Table.Width; k++)
                {
                    ...
                    {
                        int cheight = cell.Table[k, s].GetTextHeight(this, g, Font, _drawInfo, cell.Table.ColumnsWidth[k]);
                        if (height < cheight)
                        {
        ...
        protected void UpdateHeight(Graphics g)
        {
            for (int s = 0; s < _rootCell.Table.Height; s++)
            {
                ...
                {
                    _rootCell.Table.RowCount[s] = Math.Max(_rootCell.Table.RowCount[s], CountCellRows(_rootCell.Table[k, s]));
